...
module Day11 =
    ...
    let applyRulesAndUpdateMemo(memoMap : MemoMap) ((times,stone): (int64*int64)) : (MemoMap * (int64*int64) array) =
        ...
        | Some (v) -> 
            (memoMap,v |> Array.map (fun (n,i) -> (n*times, i)) )
        | None -> 
             let memoResult = 
                 [|stone|]
                 |> applyRulesManyTimes memoLength
                 |> squish
    ...
    let squishMore (stones: (int64*int64) array) : ((int64 * int64) array) = 
        stones
        |> Array.groupBy snd
        |> Array.map (fun (s,v) -> (v |> Array.map fst |> Array.sum,s))

    let rec applyRulesToStonesAndUpdateMap (memoMap: MemoMap) (stoneAcc: (int64 * int64) list) (stones:(int64 * int64) list) : MemoMap * (int64 * int64) array =
        match stones with
        | [] -> 
    ...
    let rec runRules (memoMap : MemoMap) (times: int) (stones: (int64*int64) array) : (int64 * int64) array =
        if times = 0 then
            ...
            let (newMemoMap, newStones) = applyRulesToStonesAndUpdateMap memoMap [] (List.ofArray stones)
            let numberOfStones = newStones |> Array.map fst |> Array.sum
